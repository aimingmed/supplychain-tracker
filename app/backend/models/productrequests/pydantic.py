...
class RequestDetailsSchema(BaseModel):
    ...
    requestproductid: str = Field(
        ..., max_length=20, description="需求产品号 (ProductDetails.productid foreign key)"
    )
...
class RequestDetailsCreate(BaseModel):
    ...
    requestproductid: str = Field(
        ...,
        max_length=20,
        description="需求产品号 (ProductDetails.productid foreign key)",
        example="P12345",
    )
    requestunit: int = Field(..., description="需求单位", example=1)
    is_urgent: bool = Field(default=False, description="是否紧急请求")
...
# Response schema for detailed request information
